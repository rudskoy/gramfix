...
struct PreviewPane: View {
            ...
            GeometryReader { geometry in
                VStack(spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        if item.isProcessing && !item.hasAnyPromptResult && !item.needsTranslation {
                            // All prompts still processing, none complete yet
                            processingPlaceholder(item: item)
                        } else if item.hasAnyPromptResult || item.needsTranslation {
                            // At least one result available, or translation requested
                            VStack(spacing: 0) {
                                ScrollView {
                                    if item.isSelectedTranslationProcessing {
                                        // Translation for selected target is processing
                                        TranslatingPlaceholder(targetLanguage: item.selectedTargetLanguage)
                                            .padding(14)
                                    } else if let response = item.displayedAIResponse {
                                        // Show translated or original AI response
                                        Text(response)
                                            .font(.body)
                                            ...
                                            .padding(14)
                                    } else if item.selectedPromptResult != nil {
                                        // Regular prompt result display
                                        Text(item.selectedPromptResult!)
                                            .font(.body)
                                            .foregroundStyle(.primary)
                                            .textSelection(.enabled)
                                            .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func promptTagsFooter(item: ClipboardItem) -> some View {
        HStack(alignment: .center, spacing: 8) {
            // Language flag (replaces tag icon)
            LanguageFlagView(
                item: item,
...
/// Animated placeholder shown while translation is in progress
struct TranslatingPlaceholder: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // Skeleton lines for translated content
